...
describe('Permohonan Penjaminan E2E Flow', () => {
  ...
  it('1. Permohonan Penjaminan Non Cash Loan', () => {
    ...
    cy.get('input[aria-label="NPWP"]').clear().type('9999999999999111');
    cy.contains('Pengecekan Cif').click();
    cy.wait('@inqCif');

    selectQ('Nomor PKS', '17/SE/6/VII/2024HKL');
    ...
    cy.wait(5000); // Tambahan wait untuk memastikan semua perhitungan selesai sebelum lanjut
  ...
  it('3. Verifikasi dan Analisa Penjaminan', () => {
    ...
    cy.then(() => {
      ...
      cy.contains('button', 'Next').click();
      cy.wait(2000); // Tunggu sejenak untuk memastikan semua file sudah ter-upload sebelum submit
