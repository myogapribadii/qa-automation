...
describe('Permohonan Penjaminan E2E Flow', () => {
    ...
    it('1. Permohonan Penjaminan Non Cash Loan', () => {
        cy.intercept('GET', '**/api/all-auth/master/lov/lob**').as('getLob');
        cy.intercept('GET', '**/api/all-auth/master/lov/produk**').as('getProduk');
        cy.intercept('GET', '**/api/all-auth/master/lov/subproduk-by-uker**').as('getSubProduk');
        cy.intercept('GET', '**/api/full-auth/cif/inquiry**').as('inqCif');
        ...
        cy.wait('@getLob', { timeout: 20000 });
        cy.get('input[aria-label="Lob"]')
            .should('not.be.disabled')
            .wait(500);
        selectQ('Lob', 'SURETYSHIP');

        cy.wait('@getProduk', { timeout: 20000 });
        ...
        cy.get('input[aria-label="Sub Produk"]')
            .should('not.be.disabled')
            .wait(500);
        selectQ('Sub Produk', 'Jaminan Pelaksanaan - KREDIT KONTRA BANK GARANSI', { timeout: 20000 });

        cy.wait('@getMitraPenerimaJaminan');
        cy.get('input[aria-label="Mitra"]');
        selectQ('Mitra', '008 - PT BANK MANDIRI (PERSERO) TBK', { timeout: 50000 })

        cy.get('input[aria-label="NPWP"]').should('be.visible').clear().type('9999999999999111');
        cy.contains('Pengecekan Cif').click();
        cy.wait('@inqCif');

        cy.wait('@getDcDataDasar', { timeout: 20000 });
        cy.get('input[aria-label="Nomor PKS"]')
            .should('not.be.disabled')
            .wait(500);
        selectQ('Nomor PKS', 'JMK/SIT/KBG/CAC/02112024');
