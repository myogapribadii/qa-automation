...
describe('Permohonan Penjaminan E2E Flow', () => {
    ...
    it('1. Permohonan Penjaminan Non Cash Loan', () => {
        ...
        cy.get('input[aria-label="Nomor Plafond Guarantee"]')
            ...
            .wait(500);
        selectQ('Nomor Plafond Guarantee', '10');
        ...
        cy.get('input[aria-label="PKS (Perjanjian Kerja Sama)"]')
            .should('not.be.disabled')
            .wait(500);
        selectQ('PKS (Perjanjian Kerja Sama)', 'PKS-JamkridaJatim-SB-01/2025-MemberCoguar');
        ...
        cy.get('input[aria-label="Cabang Obligee"]').parent().type('999 - ALAM SUTERA REALTY Tbk');
        cy.contains('.q-item', '999 - ALAM SUTERA REALTY Tbk').click();

        cy.get('input[aria-label="Pejabat Pembuat Komitmen (PPK)"]').type('ini merupakan nama PPK');
        cy.get('input[aria-label*="Tampilkan PPK pada SP"]').click({ force: true });
        ...
        cy.pause(); // Debugging: Pastikan perhitungan IJP sudah muncul sebelum klik next
        ...
        uploadFunc('copy surat penunjukan pemenang/SPPBJ/SPK/PO/Letter of Award');
    ...
    it('2. OTS dan Draft SP', () => {
        cy.fixture('urls').then((urls) => { cy.visit(urls.draftSpUat); });
    ...
    it('3. Verifikasi dan Analisa Penjaminan', () => {
        cy.fixture('urls').then((urls) => { cy.visit(urls.verifikasiAnalisaUat); });
    ...
    it('4. Keputusan Penjaminan', () => {
        ...
        cy.logoutAs('managerBisnisUat');

        cy.fixture('urls').then((urls) => {
            cy.visit(urls.login);
            cy.contains('Welcome to JaGuarS').should('be.visible');
            cy.loginAs('managerKlaimUat');
            cy.url({ timeout: 15000 }).should('include', '/dashboard');
        });
        ...
        cy.logoutAs('managerKlaimUat');

        cy.fixture('urls').then((urls) => {
            cy.visit(urls.login);
            cy.contains('Welcome to JaGuarS').should('be.visible');
            cy.loginAs('pemimpinCabangUat');
            cy.url({ timeout: 15000 }).should('include', '/dashboard');
        });
    ...
    it('Penerimaan IJP', () => {
        cy.fixture('urls').then((urls) => { cy.visit(urls.penerimaanIjpUat); });
    ...
    // ============================================================
    it('Penerbitan SP', () => {
        cy.fixture('urls').then((urls) => { cy.visit(urls.penerbitanSPUat); });
